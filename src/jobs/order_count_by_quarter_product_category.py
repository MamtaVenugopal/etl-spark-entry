...
def main():
    ...
    # Join dataframes
    joined_df = orders_df.join(order_items_df, 'order_id').join(products_df, 'product_id')

    # Aggregate orders by quarter and product category
    result_df = (joined_df
        .groupBy(year(col('order_purchase_timestamp')).alias('year'), quarter(col('order_purchase_timestamp')).alias('quarter'), col('product_category'))
