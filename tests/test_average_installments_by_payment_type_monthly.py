"""Auto-generated structural tests for US-1780809919436."""
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
PIPELINE = PROJECT_ROOT / "src/jobs/average_installments_by_payment_type_monthly.py"
...
def test_source_tables_referenced():
    ...
    for table in ['olist_orders_raw', 'olist_order_payments_raw']:
...
def test_target_referenced():
    text = PIPELINE.read_text(encoding="utf-8")
    assert (
        "average_installments_by_payment_type_monthly" in text
        or "gold.average_installments_by_payment_type_monthly" in text
