"""Auto-generated structural tests for US-1780245818351."""
from pathlib import Path

PROJECT_ROOT = Path(__file__).resolve().parents[1]
PIPELINE = PROJECT_ROOT / "src/jobs/product_weight_summary_by_city.py"
...
def test_source_tables_referenced():
    ...
    for table in ['olist_orders_raw', 'olist_order_items_raw', 'olist_products_raw', 'olist_customers_raw']:
...
def test_target_referenced():
    text = PIPELINE.read_text(encoding="utf-8")
    assert (
        "product_weight_per_city" in text
        or "gold.product_weight_per_city" in text
