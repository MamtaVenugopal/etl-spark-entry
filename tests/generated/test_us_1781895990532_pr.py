...
ACCEPTANCE_CRITERIA = [
  ...
  "SELECT SUM(revenue) FROM gold.monthly_revenue_summary WHERE year = 2016 AND month = 1",
  "SELECT SUM(revenue) FROM gold.monthly_revenue_summary WHERE year = 2017 AND month = 2",
  "SELECT SUM(revenue) FROM gold.monthly_revenue_summary WHERE year = 2018 AND month = 3"
]
...
def test_source_tables_referenced_in_job():
    sources = ["olist_orders_raw", "olist_order_items_raw", "olist_products_raw"]
    text = _job_text()
