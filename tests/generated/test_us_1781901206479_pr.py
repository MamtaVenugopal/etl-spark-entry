...
ACCEPTANCE_CRITERIA = [
  "SELECT year, month, product_category_name_english, SUM(payment_value) AS total_revenue FROM gold.monthly_revenue_summary WHERE year IN (2016, 2017, 2018) AND month IN (1, 2, 3) GROUP BY year, month, product_category_name_english",
  "The total_revenue for each product_category_name_english in Q1 should match the expected revenue based on the raw data"
]
